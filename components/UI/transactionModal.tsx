import React, { FunctionComponent, ReactNode, useState, useEffect, useCallback } from "react";
...
const TransactionModal: FunctionComponent<TransactionModalProps> = ({
  ...
  const closeModal = useCallback((canClose = true) => {
    if (!canClose) return;
    setIsTxSent(false);
    setIsSendingTx(false);
    handleClose();
  }, [setIsTxSent, setIsSendingTx, handleClose]);

  return (
    <Modal
      disableAutoFocus
      open={isModalOpen}
      onClose={() => (!isMobile && !isSendingTx) && closeModal()}
